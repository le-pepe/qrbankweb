...
import {getExternalAccounts, getOwnAccounts} from "@/lib/actions";
...
export default function HomePage() {
    ...
    useEffect(() => {
        ...
        getExternalAccounts().then(setExternalAccounts)
    }, [])
                            ...
                            {myAccounts.length > 0 ? (
                                myAccounts.map((account) => (
                                    <Card key={account.id} className="w-full shadow-md hover:shadow-lg transition-shadow duration-300 dark:bg-gray-800">
                                        <CardHeader>
                                            <CardTitle className="text-lg text-blue-600 dark:text-blue-400">{account.name}</CardTitle>
                                        ...
                                        <CardContent>
                                            <p className="text-xs text-gray-500 dark:text-gray-500">
                                                Creado: {new Date(account.created_at).toLocaleDateString('es-CL', { year: 'numeric', month: 'short', day: 'numeric' })}
                                            </p>
                                        </CardContent>
                                        <CardFooter className="flex flex-col sm:flex-row justify-end gap-2 pt-4">
                                            ...
                                            <ShowAccountQr account={selectedAccount} open={!!selectedAccount} onClose={() =>setSelectedAccount(null)} />
                                            {/* Aquí podrías agregar más botones de acción para cada tarjeta si es necesario */}
                                        </CardFooter>
                                    </Card>
                                ))
                            ) : (
